...
def probability(q, s, xEst, iItem, linear = False):
    vS = sum([xEst[i] * iItem[i] for i in range(len(xEst))])
...
def calculateMarginal(position, data, estX, studentSize, questionSize, nCol, linear = False):
    ...
    if not linear:
        ...
        r['Average Marginal Logistic'] = [ np.array([ logisticD(estX[x[2]], estX[x[1]], xVarEst, x[-nCol:], question, student, xVarEst[xVarPos]) for x in subData]).mean() ]
    ...
    for b in range(0, minB+1):
        r["AME k=" + str(b)] = [ np.array([ dItemPb(estX[x[2]], estX[x[1]], b, minB, xVarEst, x[-nCol:], question, student, linear, xVarEst[xVarPos]) for x in subData ]).mean() ]
