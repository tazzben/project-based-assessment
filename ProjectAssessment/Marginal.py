...
def itemPb(q, s, k, b, xEst, iItem, linear = False):
    return (probability(q, s, xEst, iItem, linear)+(probability(q, s, xEst, iItem, linear)-1)*math.ceil(-k/b))*(1-probability(q, s, xEst, iItem, linear))**(math.floor(k))
...
def dItemPb(q, s, k, b, xEst, iItem, question = False, student = False, linear = False, xVal = 0):
    ...
    return probabilityDerivative(q, s, xEst, iItem, question, student, linear, xVal)*(-1*(1-probability(q, s, xEst, iItem, linear))**(math.floor(k)-1))*(-1*(math.floor(k)+1)*math.floor(k/b)*(probability(q, s, xEst, iItem, linear)-1)+(math.floor(k)+1)*probability(q, s, xEst, iItem, linear)-1)
